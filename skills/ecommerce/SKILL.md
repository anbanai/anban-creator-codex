...
- 读取任务输入：**产品图发现 → `$PRODUCT_PHOTOS`**（服务端把上传产品图下载到 `$DIR/.anban-creator/products/`，读 `index.json` 拼路径；= `get_project_profile` 的 `ecommerce.product_photo_dir`，期望数见 `product_photo_count`；`index.json` 缺失或全无可访问 → 停止并请求上传）、`selected_modules`、`target_platform`、`selling_points`、`visual_style`、语言。（图像模型已在建任务时选定，经 `image_model` 读取，不再在此覆盖。）
