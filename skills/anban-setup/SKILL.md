...
2. **次选方案**：如果用户希望走 `~/.codex/config.toml`，提示其手动追加以下段（先 Read 现有文件确认无重复，再用 Edit 合并，绝不覆盖）：
   ...
   ```
   并在 shell 中保留 `export ANBAN_API_KEY="..."`（Codex 的 `bearer_token_env_var` 读取的是环境变量，不是 config.toml 内联值）。
...
> 配置完成。**请退出并重新启动 Codex**，让 MCP 连接生效。重启后再次运行 `$anban-setup` 验证连接。

## 重启后验证

用户重启 Codex 后，`$anban-setup` 的预检步骤应自动执行。预期结果：
